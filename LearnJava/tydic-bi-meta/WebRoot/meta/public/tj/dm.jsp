...
dnm.innerHTML+=" <br/> navigator.appMinorVersion 	= "+navigator.appMinorVersion 		;//获取应用程序的次版本值。 
dnm.innerHTML+=" <br/> navigator.appName 			= "+navigator.appName 				;//获取浏览器的名称。 
dnm.innerHTML+=" <br/> navigator.appVersion 		= "+navigator.appVersion 				;//获取浏览器运行的平台和版本。 
dnm.innerHTML+=" <br/> navigator.browserLanguage 	= "+navigator.browserLanguage 		;//获取浏览器的当前语言。 
dnm.innerHTML+=" <br/> navigator.cookieEnabled 	    = "+navigator.cookieEnabled 			;//获取客户端的永久 cookie 是否在浏览器中启用。永久 cookies 是储存在客户端计算机上的。 
dnm.innerHTML+=" <br/> navigator.cpuClass 		    = "+navigator.cpuClass 				;//获取指示 CPU 等级的字符串。 
dnm.innerHTML+=" <br/> navigator.onLine 			= "+navigator.onLine 					;//获取表明系统是否处于全局脱机模式的值。 
dnm.innerHTML+=" <br/> navigator.platform 		    = "+navigator.platform 				;//获取用户的操作系统名称。 
dnm.innerHTML+=" <br/> navigator.systemLanguage 	= "+navigator.systemLanguage 			;//获取操作系统适用的默认语言。 
dnm.innerHTML+=" <br/> navigator.userAgent 		    = "+navigator.userAgent 				;//获取等同于 HTTP 用户代理请求头的字符串。 
dnm.innerHTML+=" <br/> navigator.userLanguage 	    = "+navigator.userLanguage 			;//获取操作系统的自然语言设置。
dnm.innerHTML+=" <br/> navigator.appCodeName 		= "+navigator.appCodeName 			;//获取浏览器的代码名称。 
dnm.innerHTML+=" <br/> ";
dnm.innerHTML+=" <br/> ";
dnm.innerHTML+=" 360浏览器标识："+(external.twGetVersion(external.twGetSecurityID(window)) &&
            external.twGetRunPath.toLowerCase().indexOf("360se") > -1 && (a = 1),a); 
