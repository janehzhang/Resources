...
dwrCaller.addAutoAction("queryTableDataSourceByDim", "TableApplyAction.queryTableDataSourceByCurId");
dwrCaller.addDataConverter("queryTableDataSourceByDim", tableDataSourceConverter);
...
var applyInit = function(){
    ...
    applyLayout.cells("a").setHeight(230);
...
//    applyLayout.cells("b").setText("编辑列属性");
    ...
    $("_clumnContentDiv").style.height = (document.body.offsetHeight - 310) + "px";
    ...
    Tools.addEvent($("_tableType"), "change", function(){
        ...
        if(tableType == 3){
		    ...
		    dwrCaller.executeAction("queryTableDataSourceByDim", function(data){
				document.getElementById("_dataSource").length = 0;
		    	document.getElementById("_tableOwner").length = 0;
				document.getElementById("_tableSpace").length = 0;
				Tools.addOption("_dataSource", [{value:"",text:"--请选择--"}]);
...
var addColumnRow = function(rowIndex,isShowOper){
    ...
    if(!$("_columnRow" + rowIndex)){
        ...
        input = dhx.html.create("input", {name:"colBusComment",id:"_colBusComment" +
                                                                  rowIndex,className:"dhxlist_txt_textarea",type:"TEXT",style:"width: 65%;height:15px;margin-bottom:4px;line-height:15px;"});
        td.appendChild(input);
...
var genSql=function(){
    if(validate()){
        ...
        dwrCaller.executeAction("generateSql",data,function(sql){
            var dhxWindow = new dhtmlXWindows();
            dhxWindow.createWindow("sqlWindow", 0, 0, 300, 360);
            var sqlWindow = dhxWindow.window("sqlWindow");
            sqlWindow.stick();
            sqlWindow.setModal(true);
            sqlWindow.setDimension(300, 360);
...
//        layout.cells("a").setHeight(Math.ceil(winsize.height/7)*3);
        ...
        */
        ...
        //表字段关联属性设置
        var div=document.createElement("div");
        div.innerHTML='<table cellpadding="0" cellspacing="0" style="width:100%;height: 85%;position: relative;">' +
...
//        (!dhx.env.isIE||parseInt(dhx.env.ie)>7)&&(cancel.style.marginTop = "-10px");
        ...
        var radioCheck=function(rId){
            queryColumnGrid.clearAll();
            queryColumnGrid.getCheckedRows(0);
...
dwrCaller.isShowProcess("queryDbTableColumns",true);
...
var importFromDatabase=function(){
    ...
    if(!importWindowFromDb){
        var dhxWindow = new dhtmlXWindows();
        var winsize=Tools.propWidthDycSize(12,20,12,16);
        dhxWindow.createWindow("importWindowFromDb", 0, 0, 450, 430);
...
//        alert(Math.ceil(winsize.height/7)*3);
        layout.cells("a").setHeight(200);
        layout.cells("a").fixSize(false, true);
        layout.cells("a").firstChild.style.height = (layout.cells("a").firstChild.offsetHeight + 5) + "px";
        layout.cells("a").hideHeader();
        ...
        div.innerHTML='<table cellpadding="0" cellspacing="0" style="width:100%;height:85%;position: relative;">' +
                      ...
                      '<tr style="height: 15%;"><td style="height: 15%;border-top: #99b2c9 solid 1px;"  id="_queryFromWinButtonFromDb"></td></tr></table>';
        layout.cells("b").attachObject(div.lastChild);
...
//        (!dhx.env.isIE||parseInt(dhx.env.ie)>7)&&(ok.style.marginTop = "-10px");
        ...
        ok.onclick=function(){
            ...
            //获取table信息
            var tableData=queryTableGrid.getRowData(selectTableId).userdata;
            tableData.dataSourceId = $("_tableDataSource").value;
            tableData.tableOwener = $("_owner").value;
            //获取列信息
            var selectedColumnIds=queryColumnGrid.getCheckedRows(0);
            var columnDatas=[];
            if(selectedColumnIds){
...
var copyTablData=function(tableData,columnDatas){
    ...
    if(tableData.dataSourceId||tableData.dataSourceId==0){
        ...
        if(data){
            if(!(data.options.length==1 && !data.options[0].text)){
                ...
                var temp = document.getElementById("_tableOwner");
                for(var i=0;i<temp.options.length;i++){
                	if(temp.options[i].text == tableData.tableOwner ){
                        if($("_tableName").value.toUpperCase().indexOf("{USER}")==-1){
               			    document.getElementById("_tableOwner").options[i].selected = true;
							ownerChange(document.getElementById("_tableOwner").options[i]);
							checkOwner = "";
                        }else{
                            $('_tableOwner').value = "";
                        }
...
var copyColumnData=function(data,rowIndex){
    ...
    if(data.colBusType == 0){
    	if(data.dimTableName!=undefined){
...
var initColData=[
    ...
    {colName:"SRC_PAR_CODE",datatype:"VARCHAR2(100)",colBusType:1,colNullabled:1,isPrimary:0,colBusComment:"父编码",columAttribute:2,colNameCn:"父编码"},
