...
//    $("_columnForm").style.height=($("_columnFormDiv").offsetHeight-30)/2+"px";
    ...
    dhtmlxValidation.addValidation($("_baseInfoForm"), [
        ...
        {target:"_tableDimPrefix",rule:"NotEmpty,MaxLength[12],ValidAplhaNumeric"},
...
var initInfo = function (){
	 //加载所有的表类数据源
   // TableApplyAction.queryTableDataSource( function(data){
     //   dataSourceData=data;
     //   Tools.addOption("_dataSource", data);
    //})
    
    TableApplyAction.queryTableDataSource({
        ...
        dwrCaller.executeAction("queryAllColumnsByTableID",tableVersion,function(columnDatas){
            if(columnDatas&&columnDatas.length>0){
                for(var i=0;i<columnDatas.length;i++){
                    ...
                    if(initCol || (colsName.length > 0 && !canModifyDimCol)){
                    ...
                    $("_isPrimary"+columnNum).setAttribute("disabled",true);
                    addColumnRow(null,true,colsName);
                }
            }
        })
        //查询DIM TYPE
...
//维度基本列信息
var initColData=["ID","PAR_ID","PAR_CODE","CODE","NAME","DESC","DIM_TABLE_ID","DIM_TYPE_ID","STATE","DIM_LEVEL","MOD_FLAG","ORDER_ID"];
...
var addColumnRow = function(rowIndex,isShowOper){
    ...
    if(!$("_columnRow" + rowIndex)){
            ...
            build:function(data){
            	if(data.options.length > 0){
	                ...
	                if(this._options && this._options.length > 1){
	                    for(var i = 0; i < this._options.length; i++){
	                    	 this._options[i].userdata.colBusComment = (this._options[i].userdata.colBusComment == null)?"":this._options[i].userdata.colBusComment;
	                        contentHtml += '<tr id="' + this.getSelectUniqueName(i) + '" style="' + this.style.unSelect +
	                        ...
	                        contentHtml += '<td style="text-align: left;">' + this._options[i].userdata.datatype +
	                                       '</td>';
	                        contentHtml += '<td style="text-align: left;">' + this._options[i].userdata.colBusComment  +
	                                       '</td>';
	                        contentHtml += "</tr>";
	                    }
	                } else{
        ...
        input = dhx.html.create("input", {name:"colBusComment",id:"_colBusComment" +
                                                                  rowIndex,className:"dhxlist_txt_textarea",type:"TEXT",style:"width: 65%;"});
        td.appendChild(input);
...
//                }
        ...
        //列属性处理
        if(rowData.colName == tableData.tableDimPrefix.toUpperCase() + "_ID"){ //子节点
            metaDimTable.dimKeyColId=rowData.colName;
        }else if(rowData.colName == tableData.tableDimPrefix.toUpperCase() + "_PAR_ID"){ //父节点
...
var submitValidate=function(){
    if(validate(true)){
        ...
        dwrCaller.executeAction("validate",data,function(validateResult){
            ...
            validateLayout.cells("a").setHeight(350);
            ...
            save.onclick=function(){
                var insertTable=function(){
                    dwrCaller.executeAction("updateDim",data,function(rs){
                        if(rs.result == "true"){
                        	if(rs.exception) {
                            	dhx.alert("修改表数据成功！但是同步表结构失败！你可以重新修改表类！详细信息："+rs.exception,null,{width:'400',height:'200'});
                            }else {
                            	dhx.alert("修改成功！");
                            }
                            tableVersion = tableVersion+1;
                            if(window.parent&&	window.parent.tableVersion){
                                                       	window.parent.tableVersion = tableVersion;
...
//        tableData.tableOwner&&dwr.util.setValue($("_tableOwner"),tableData.tableOwner);
       ...
       dwrCaller.executeAction("queryTableSpace",tableData.dataSourceId,function(data){
		   ...
		   if(data){
		   	...
		   	document.getElementById("_tableSpace").value = tableData.tableSpace;
	        dhx.env.isIE&&CollectGarbage();
	    });
