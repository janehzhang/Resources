...
var tblMacro = ['{YY}', '{YYYY}', '{MM}', '{DD}', '{HH}', '{MI}', '{SS}', '{M}', '{D}', '{YYYYMM}', '{YYYYMMN}', '{YYYYMMP}',
    ...
    '{N_YYYYMM}', '{N_YYYYMMN}', '{N_YYYYMMP}', '{N_YYYYMMDD}', '{N_YYYYMMDDP}', '{LOCAL_CODE}', '{LOCAL_NAME}', '{USER}'];
...
var tblNameCheck = function (value) {
    ...
    var res = reg.test(value.toUpperCase()) ? true : "表类名不符合规范！";
    if (res != true) {
        return res;
    }
    if (value.length > 30) {
    ...
    //带{USER}宏变量的表不能选择所属用户
    if (value.toUpperCase().indexOf("{USER}") > -1) {
        $('_tableOwner').value = "";
        $('_tableOwner').disabled = "true";
    } else {
        $('_tableOwner').disabled = "";
        $('_tableOwner').value = tmpOwner;
    }
    ...
    userData:function (rowData) {
        if (checkOwner == "") {
            if (rowData.defaultName) {
                checkOwner = rowData.defaultName;
            }
        }
        if (checkSpace == "") {
            if (rowData.defaultTablespace) {
                checkSpace = rowData.defaultTablespace;
            }
        }
        return rowData;
    }
...
var dataSourceChange = function (obj) {
    ...
    dwrCaller.executeAction("getUserNameByDataSourceId", obj.value, function (data) {
        document.getElementById("_tableOwner").innerHTML = '<option value=""></option>';
//        Tools.addOption("_tableOwner", [{value:"",text:"--请选择--"}]);
        if (data) {
            if (!(data.options.length == 1 && !data.options[0].text)) {
                Tools.addOption("_tableOwner", data);
                var temp = document.getElementById("_tableOwner");
                for (var i = 0; i < temp.options.length; i++) {
                    if (temp.options[i].text == checkOwner) {
                        if ($("_tableName").value.toUpperCase().indexOf("{USER}") == -1) {
                            document.getElementById("_tableOwner").options[i].selected = true;
                            tmpOwner = checkOwner;
                        } else {
                            $('_tableOwner').value = "";
                        }
            ...
            if (data) {
                if (!(data.options.length == 1 && !data.options[0].text)) {
                    ...
                    for (var i = 0; i < data.options.length; i++) {
                        if (data.options[i].text == checkSpace) {
                            document.getElementById("_tableSpace").options[i].selected = true;
                            checkSpace = "";
                            break;
                        }
                    }
...
/*
 var dataSourceChange = function(obj){
 TableApplyAction.getUserNameByDataSourceId(obj.value, function(data){
 document.getElementById("_tableOwner").value=data;
 });
 }
 */
...
dwrCaller.addAutoAction("queryTableDataSource", "TableApplyAction.queryTableDataSourceByCurId");
dwrCaller.addDataConverter("queryTableDataSource", tableDataSourceConverter);
dwrCaller.isShowProcess("queryTableDataSource", false);

// 从数据表中导入的表类数据源相关Action
dwrCaller.addAutoAction("queryTableDataSourceIsImportDB", "TableApplyAction.queryTableDataSource");
dwrCaller.addDataConverter("queryTableDataSourceIsImportDB", tableDataSourceConverter);
dwrCaller.isShowProcess("queryTableDataSourceIsImportDB", false);
...
var dimApplyInit = function () {
    ...
    applyLayout.cells("a").setText("申请维度表类");
...
//    $("_columnForm").style.height=($("_columnFormDiv").offsetHeight-30)/2+"px";
    $("_clumnContentDiv").style.height = ($("_columnFormDiv").clientHeight - 80) / 2 + "px";
    ...
    if (dhx.env.isIE && parseInt(dhx.env.ie) <= 7) {
        $("_columnDimForm").style.marginTop = "-18px";
    }
    //设置CSS
    ...
    //表分类下拉树数据准备。
    var tableGroupTree = new loadTableGroupTree3($("_tableGroup"), $("_tableGroupId"));
    //为_tableGroup添加事件
    ...
    dwrCaller.executeAction("queryTableDataSource", function (data) {
        ...
        dwrCaller.executeAction("getUserNameByDataSourceId", dwr.util.getValue("_dataSource"), function (data) {
            document.getElementById("_tableOwner").innerHTML = '<option value=""></option>';
//        Tools.addOption("_tableOwner", [{value:"",text:"--请选择--"}]);
            if (data) {
                if (!(data.options.length == 1 && !data.options[0].text)) {
            ...
            dwrCaller.executeAction("queryTableSpace", 0, function (data) {
                document.getElementById("_tableSpace").innerHTML = "";
//        Tools.addOption("_tableSpace", [{value:"",text:"--请选择--"}]);
                if (data) {
                    if (!(data.options.length == 1 && !data.options[0].text)) {
            ...
            if (data.options.length > 0) {
                ...
                if (this._options && this._options.length > 0) {
                    for (var i = 0; i < this._options.length && i < 15; i++) {
                        contentHtml += '<tr style="text-align: left;" id="' + this.getSelectUniqueName(i) + '" style="' + this.style.unSelect +
    ...
    dhtmlxValidation.addValidation($("_baseInfoForm"), [
        ...
        {target:"_tableDimPrefix", rule:"NotEmpty,MaxLength[23],ValidAplhaNumeric"},
...
//维度基本列信息
var initColData = [
    {colName:"ID", datatype:"NUMBER(18)", colNullabled:0, isPrimary:1, colBusComment:"维度表唯一标识", columAttribute:1, colNameCn:"维度表ID"},
    {colName:"PAR_ID", datatype:"NUMBER(18)", colNullabled:0, isPrimary:0, colBusComment:"维度表父ID", columAttribute:2, colNameCn:"维度表父ID"},
    {colName:"CODE", datatype:"VARCHAR2(100)", colNullabled:0, isPrimary:0, colBusComment:"业务编码字段", columAttribute:0, colNameCn:"业务编码字段"},
    {colName:"PAR_CODE", datatype:"VARCHAR2(100)", colNullabled:0, isPrimary:0, colBusComment:"父业务编码字段", columAttribute:0, colNameCn:"父业务编码字段"},
    {colName:"NAME", datatype:"VARCHAR2(500)", colNullabled:0, isPrimary:0, colBusComment:"业务含义", columAttribute:0, colNameCn:"业务含义"},
    {colName:"DESC", datatype:"VARCHAR2(1000)", colNullabled:1, isPrimary:0, colBusComment:"业务描述", columAttribute:0, colNameCn:"业务描述"},
    {colName:"DIM_TABLE_ID", datatype:"NUMBER(18)", colNullabled:0, isPrimary:0, colBusComment:"维度表类ID", columAttribute:0, colNameCn:"维度表类ID"},
    {colName:"DIM_TYPE_ID", datatype:"NUMBER(18)", colNullabled:0, isPrimary:0, colBusComment:"维度分组类型", columAttribute:0, colNameCn:"维度分组类型"},
    {colName:"STATE", datatype:"NUMBER(18)", colNullabled:0, isPrimary:0, colBusComment:"有效状态 0 无效 1有效", columAttribute:0, colNameCn:"有效状态"},
    {colName:"DIM_LEVEL", datatype:"NUMBER(18)", colNullabled:0, isPrimary:0, colBusComment:"维度层级", columAttribute:0, colNameCn:"维度层级"},
    {colName:"MOD_FLAG", datatype:"NUMBER(18)", colNullabled:0, isPrimary:0, colBusComment:"修改标识  0正常  1新增 2修改", columAttribute:0, colNameCn:"修改标识"},
    {colName:"ORDER_ID", datatype:"NUMBER(18)", colNullabled:0, isPrimary:0, colBusComment:"排序ID", columAttribute:0, colNameCn:"排序ID"}
]
...
var initCol = function () {
    for (var i = 0; i < initColData.length; i++) {
        ...
        $("_dimInfo_img" + columnNum).setAttribute("disabled", true);
        initRowIndex[initColData[i].colName] = columnNum;
...
var resertColName = function () {
    ...
    for (var i = 0; i < 6; i++) {
        var input = $("_colName" + initRowIndex[initColData[i].colName]);
        var columnName = prex ? (prex.toUpperCase() + "_" + initColData[i].colName) : initColData[i].colName
...
var treeData2 = {};
var lastTableType2 = null;//最后一次显示时的tableType.
/**
 * modify:tanwc  解决业务类型和从原数据表复制中的业务类型关联影响BUG、
 * @param {Object} target
 * @param {Object} hide
 * @memberOf {TypeName}
 */
...
var treeData3 = {};
var lastTableType3 = null;//最后一次显示时的tableType.
/**
 * modify:tanwc  解决业务类型和从原数据表复制中的业务类型关联影响BUG、
 * @param {Object} target
 * @param {Object} hide
 * @memberOf {TypeName}
 */
var loadTableGroupTree3 = function (target, hide) {
    //创建tree Div层
    var div = dhx.html.create("div", {
        style:"display;none;position:absolute;border: 1px #eee solid;height: 200px;overflow: auto;padding: 0;margin: 0;" +
            "z-index:1000"
    ...
    this.showGroupTree = function (tableType) {
        var show = function () {
            if (lastTableType3 != tableType) {
                ...
                tree.loadJSONObject(treeData3[tableType]);
            }
        }
        if (!treeData3[tableType]) {
            dwrCaller.executeAction("queryTableGroup", tableType, function (data) {
                treeData3[tableType] = data;
                show();
                lastTableType3 = tableType;
            })
        } else {
            show();
            lastTableType3 = tableType;
...
var collapse = function (div, collapseContent, extend) {
    ...
    if (!$(collapseContent)._orgHeight) {
        $(collapseContent)._orgHeight = $(collapseContent).parentNode.offsetHeight - 30;
    }
    if (!$(extend)._orgHeight) {
        $(extend)._orgHeight = $(extend).parentNode.offsetHeight - 30;
    }
    if (isOpen) {//是展开的，则收缩。
...
var dataTypeData = getCodeComboByType("DATA_TYPE");
;//缓存DATATYPE数据
...
var addColumnRow = function (rowIndex, isShowOper) {
    ...
    if (!$("_columnRow" + rowIndex)) {
        ...
        td.align = "center";
        input = dhx.html.create("input",
            {name:"colNullabled", id:"_colNullabled" + rowIndex, checked:"checked", type:"checkbox", style:"width: 90%;"})
        input.onclick = function (e) {
            e = e || window.event;
            var target = e.target || e.srcElement;
            var clickIndex = target.parentNode.parentNode._index;
        ...
        td = tr.insertCell(7);
        td.align = "center";
        td.innerHTML = '<input type="text" id="_dimInfo' + rowIndex + '" name="dimInfo" class="dhxlist_txt_textarea" readOnly="true" style="width:70%;height:15px;line-height:15px;" onclick="selectDim(this)">';
        td.innerHTML += '<img id="_dimInfo_img' + rowIndex + '" src="../../../resource/images/cancel.png" style="width:12px;height:12px;" title="清除维度信息" onclick="clearComment(this,' + rowIndex + ')">';
...
var selectDim = function (input) {
    ...
    if (!dimWindow) {
        ...
        dimLayout.cells("a").setHeight(200);
...
var addDimColumnRow = function (rowIndex) {
    ...
    if (!$("_dimColumnRow" + rowIndex)) {
        ...
        var td = tr.insertCell(0);
        td.align = "center";
        var input = dhx.html.create("input", {className:"dhxlist_txt_textarea", name:"dimTypeName", id:"_dimTypeName" + rowIndex, type:"TEXT", style:"width: 90%;height:15px;line-height:15px;"});
        //添加事件，开始输入时，新增一行。
        Tools.addEvent(input, "keyup", function (e) {
        ...
        td = tr.insertCell(2);
        td.align = "center";
        input = dhx.html.create("input", {className:"dhxlist_txt_textarea", name:"dimTypeDesc", id:"_dimTypeDesc" + rowIndex, type:"TEXT", style:"width: 90%;height:15px;line-height:15px;"});
        td.appendChild(input);

        //层级列
        td = tr.insertCell(3);
        td.align = "center";
        td.innerHTML = '<input type="text" id="_level' + rowIndex + '" name="level" class="dhxlist_txt_textarea" readOnly="true" style="width:90%;height:15px;line-height:15px;" onclick="selectLevel(this)">';
...
dwrCaller.isShowProcess("queryColumnsByTableID", false);
//地域维度层级定义窗口
...
var selectLevel = function (input) {
    ...
    if (!levelWindow["levelWindow" + rowIndex]) {
        ...
        levelWindow["levelWindow" + rowIndex].setDimension(500, 400);
        ...
        ok.onclick = function () {
            ...
            for (var i = 1; i < tableObj.rows.length - 1; i++) {
                ...
                if (!(validateRes = dhtmlxValidation.validate(tableObj.rows[i]))) {
                    break;
                }
...
var getData = function (isDim) {
    ...
    if (document.getElementById("_lastLevelFlag").checked == true) {
        tableData.lastLevelFlag = 1;
    } else if (document.getElementById("_lastLevelFlag").checked == false) {
...
var validate = function (validateDimInfo) {
    ...
    if ($("_tableName").value.toUpperCase().indexOf("{USER}") == -1 && $("_tableOwner").value == "") {
        dhx.alert("请填写所属用户！");
        $('_tableOwner').disabled = "";
        return false;
    } else if ($("_tableName").value.toUpperCase().indexOf("{USER}") > -1) {
        $("_tableOwner").disabled = "true";
    }
    for (var i = 1; i < tableObj.rows.length - 1; i++) {
        validateRes = dhtmlxValidation.validate(tableObj.rows[i]) && validateRes;
...
var genSql = function () {
    if (validate()) {
        ...
        dwrCaller.executeAction("generateSql", data, function (sql) {
            ...
            sqlWindow.setDimension(550, 400);
...
var importFormTable = function () {
    ...
    if (!importWindow) {
        var dhxWindow = new dhtmlXWindows();
        dhxWindow.createWindow("importWindow", 0, 0, 450, 440);
        var winsize = Tools.propWidthDycSize(12, 20, 12, 16);
        importWindow = dhxWindow.window("importWindow");
        importWindow.stick();
        importWindow.setModal(true);
        importWindow.setDimension(winsize.width, winsize.height + 50);
...
//        layout.cells("a").setHeight(Math.ceil(winsize.height/7)*3);
        ...
        //表字段关联属性设置
        var div = document.createElement("div");
        div.innerHTML = '<table cellpadding="0" cellspacing="0" style="width:100%;height: 95%;position: relative;">' +
            ...
            '<tr style="height: 15%"><td  id="_queryFromWinButton" style="border-top: #99b2c9 solid 1px;"></td></tr></table>';
...
dwrCaller.isShowProcess("queryDbTableColumns", false);
...
var importFromDatabase = function () {
    ...
    if (!importWindowFromDb) {
        var dhxWindow = new dhtmlXWindows();
        var winsize = Tools.propWidthDycSize(12, 20, 12, 16);
        dhxWindow.createWindow("importWindowFromDb", 0, 0, 450, 440);
        importWindowFromDb = dhxWindow.window("importWindowFromDb");
        importWindowFromDb.stick();
        importWindowFromDb.setModal(true);
        importWindowFromDb.setDimension(winsize.width, winsize.height + 50);
...
//        layout.cells("a").setHeight(Math.ceil(winsize.height/7)*3);
        ...
        //从数据表中导入的数据源装载
        dwrCaller.executeAction("queryTableDataSourceIsImportDB", function (data) {
        ...
        div.innerHTML = '<table cellpadding="0" cellspacing="0" style="width:100%;height: 95%;position: relative;">' +
            ...
            '<tr style="height: 15%"><td  id="_queryFromWinButtonFromDb" style="border-top: #99b2c9 solid 1px;"></td></tr></table>';
        layout.cells("b").attachObject(div.lastChild);
...
//        (!dhx.env.isIE||parseInt(dhx.env.ie)>7)&&(ok.style.marginTop = "-10px");
        ...
        ok.onclick = function () {
            ...
            if (selectedColumnIds) {
                var columns = selectedColumnIds.split(",");
                for (var i = 0; i < columns.length; i++) {
                    var obj = queryColumnGrid.getRowData(columns[i]).userdata;
                    try {
                        TableApplyAction.queryColNameCnAndColId(tableData.tableName, tableData.dataSourceID, obj.colName, {
                            async:false,
                            callback:function (data) {
                                obj.colNameCn = data.COL_NAME_CN;
                                //obj.dimColId = data.COL_ID;
                            }
                        });
                    } catch (e) {
                        obj.colNameCn = null;
                    ...
                    columnDatas.push(obj);
                }
            }
            //查询分区SQL
            TableApplyAction.queryPartitionSql(tableData.tableName, tableData.tableOwner, tableData.dataSourceId, function (sql) {
                tableData.partitionSql = sql;
                copyTablData(tableData, columnDatas);
            })
...
var copyTablData = function (tableData, columnDatas) {
    ...
    // 获得已经存在的列名称
    var rowDataNames = getColumnDataExists();
    if (columnDatas && columnDatas.length > 0) {
        for (var i = 0; i < columnDatas.length; i++) {
            var count = 0;
            var columnData = columnDatas[i];
            for (var j = 0; j < rowDataNames.length; j++) {
                if (columnData.colName == rowDataNames[j]) {
                    count++;
                }
            }
            if (count == 0) {
...
// 获得已存在所有列
var getColumnDataExists = function () {
    // 字段表对象
    var tableObj = $("_clumnContentTable");
    var rowDatas = new Array();
    for (var i = 1; i < tableObj.rows.length - 1; i++) {
        var rowColName = document.getElementById("_colName" + tableObj.rows[i]._index).value.toUpperCase();
        rowDatas[i - 1] = rowColName;
    }
    return rowDatas;
}
/**
...
var copyColumnData = function (data, rowIndex) {
    var colNameCn = data.colNameCn;
    //alert("abc".substring(0,10));
    try {
        if (!data.colNameCn) {
            colNameCn = data.colBusComment.substring(0, 32);
            //colNameCn = data.colBusComment;
        }
    } catch (e) {
        colNameCn = "";
    }
    ...
    dwr.util.setValue("_colNameCn" + rowIndex, colNameCn);
...
var removeAllColumnData = function () {
    var tableObj = $("_clumnContentTable");
