...
var viewInit = function(){
    var viewLayout = new dhtmlXLayoutObject(document.body, "2E");
    viewLayout.cells("a").setText("查看维度表信息");
    viewLayout.cells("b").hideHeader();
    viewLayout.cells("a").setHeight(80);
    viewLayout.cells("a").fixSize(false, true);
    viewLayout.hideConcentrate();
    viewLayout.hideSpliter();//移除分界边框。

    //加载查询表单
    var queryform = viewLayout.cells("a").attachForm([
        {type:"settings",position: "label-left", labelWidth: 55, inputWidth: 120},
        {type:"input",label:"维度表类：",name:"tableGroup",inputHeight : 17},
        {type:"newcolumn"},
        {type:"input",label:"关键字：",labelWidth: 43,name:"keyWord",inputHeight : 17},
        {type:"newcolumn"},
    ...
    //操作列按钮
    var buttons = {
        viewDim:{name:"viewDim",text:"查看维度表",imgEnabled:base + "/meta/resource/images/transparent.gif",
            imgDisabled:base + "/meta/resource/images/transparent.gif",onclick:function(rowData){
                var url="/meta/module/tbl/dim/viewDim.jsp?tableId="+rowData.id+"&tableVersion="+rowData.userdata.tableVersion;
                window.parent.closeTab("查看维度表");
    ...
    //过滤列按钮摆放
//    var buttonRoleCol = getRoleButtons();
    //getRoleButtonsCol
...
var showPopUpWindow = function(value, rowData){
    openMenu("查看维度:"+rowData.data[0], "/meta/module/dim/view/viewDetail.jsp?focus=" + value + "&tableId=" + rowData.id + "&tableName=" +
                                      rowData.data[0]+"&tableOwner="+rowData.data[4]+"&tableVersion="+rowData.userdata.tableVersion+"&tableNameCn="+encodeURIComponent(encodeURIComponent(rowData.userdata.tableNameCn)), "top");
}
dhx.ready(viewInit);