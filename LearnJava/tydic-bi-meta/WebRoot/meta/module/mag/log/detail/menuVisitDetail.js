...
var excuteInitData=function(){
     ...
     LoginLogAction.getMenuVisitDetail(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
...
function buildTable(data){
	     tableStr ="<table  width='100%' height='auto'  border='0' cellpadding='0' cellspacing='0' class='tab_01'>";  
            tableStr +="<tr>";
            	tableStr +="<td nowrap bgcolor='#cde5fd'><span class='title'>菜单</span></td>";
                  tableStr +="<td nowrap bgcolor='#cde5fd'><span class='title'>用户名</span></td>";
                  tableStr +="<td nowrap bgcolor='#cde5fd'><span class='title'>地区</span></td>";
                  tableStr +="<td nowrap bgcolor='#cde5fd'><span class='title'>部门</span></td>";
                  tableStr +="<td nowrap bgcolor='#cde5fd'><span class='title'>岗位</span></td>";
		   ...
		   tableStr +="<td  align='left'>"+data[i].MENU_NAME+"</td>"
			   		+"<td  align='left'>"+data[i].USER_NAMECN+"</td>"
		             +"<td>"+data[i].SHOW_ZONE_NAME+"</td>"
				     +"<td>"+data[i].DEPT_NAME+"</td>"
				     +"<td>"+data[i].STATION_NAME+"</td>"
				     +"<td align='left'>"+data[i].ROLE_NAME+"</td>"
				     +"<td align='left'>"+data[i].VISIT_TIME+"</td>"
				  +"</tr>";
