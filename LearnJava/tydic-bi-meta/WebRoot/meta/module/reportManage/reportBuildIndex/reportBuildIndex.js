...
var reportBuildIndex = function() {
	...
	this.replacePopularDataTemplate = function(issueId,tableAlias,count,issueNote,cols) {
		var random = Math.floor(Math.random() * ( 10000 + 1));
		var result = r_self.template4RecommendData.replaceAll('{issueId}',issueId+'_'+random);
		result = result.replaceAll('{tableAlias}',tableAlias);
		result = result.replaceAll('{id}',issueId);
	...
	this.recommendData = {
		...
		loadData:function() {
			var self = this;
			r_self.dwrCaller.executeAction('queryRecommendData',function(data){
				var html = "";
				if(data == null) {
	...
	this.popularData = {
		...
		loadData:function() {
			var self = this;
			r_self.dwrCaller.executeAction('queryPopularData',function(data){
				var html = "";
				if(data == null) {
					$('popularDataContainer').innerHTML = html;
					return;
				}
				for(var i=0;i<data.length;i++) {
					var issueId = data[i].ISSUE_ID;
	...
	this.newestData = {
		...
		loadData:function() {
			var self = this;
			r_self.dwrCaller.executeAction('queryNewestData',function(data){
				var html = "";
				if(data == null) {
...
function recommendData2More() {
	openMenu("推荐数据","/meta/module/reportManage/reportBuildIndex/recommendData2More.jsp","top",null,true);
}
function popularData2More() {
	openMenu("人气最旺数据","/meta/module/reportManage/reportBuildIndex/popularData2More.jsp","top",null,true);
}
function newestData2More() {
	openMenu("新鲜出炉数据","/meta/module/reportManage/reportBuildIndex/newestData2More.jsp","top"),null,true;
}
