...
var ServiceProblemInit = function() {
	...
	var buttonRole = [ "addServiceProblemInfo","dealServiceProblem","queryServiceProblemDetailInfoByID"];
    //过滤显示顶部按钮
    var bottonRoleRow = [];
	...
	window["getDetailButtons"] = function() {
		...
		for ( var i = 0; i < buttonRoleCol.length; i++) {
			...
			}else if(buttonRoleCol[i] != "queryServiceProblemDetailInfoByID"){
	...
	logGrid.setColumnIds("theme,problemDescription,source,urgencyDegree,processLimited,problemType,createTime,createActor,isEvaluationPlan,attachmentName,target");
...
dwrCaller.addAutoAction("affirmServiceProblem","SerProManageAction.affirmServiceProblem");
