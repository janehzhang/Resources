...
//var dateTime= $("dateTime").value;
 //定义另外的函数方法
dwrCaller.addAutoAction("queryAllSubMenu","ChannelIndexAction.queryAllSubMenu","116001",user.userId);
	 ...
	 if(data){
	    ...
	    var menuName="";
	 	 ...
	 	 for(var i=0;i<data.length;i++){
	 	  str +="<li class='line'><a href='#' menuName="+data[i].MENU_NAME+" menuId="+data[i].MENU_ID+" menuUrl="+data[i].MENU_URL+"><img src="+data[i].IMG_URL+" width='120' height='120' /><p>"+data[i].MENU_NAME+"</p></a></li>";
	 	 	if(i==0){
		...
		$(function(){ 
		    ...
		    $('.line a').bind('click', function() {
		      	...
		      	if(menuId == "11601302" ){
		      		parent.openTreeTab(menuId,menuName, base+'/'+menuUrl, 'top');
		      	}else{
		      		subMenuData(menuId,menuName);
				     $("#rightId").empty();
				     //subMenuDataNo(menuId,menuName);	
