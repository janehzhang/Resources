...
var selectTable=function(map)
{
     ...
     CmplGxbMonAction.getTableData(map, function (res) {
    	              dhx.closeProgress();
				        if (res == null) {
...
var openMenuHref=function(zoneName,zoneCode){
	   var dateTime=$("dateTime").value;
	   var url="/portalCommon/module/procedure/ts/skip/list/cmplGxbDetail.jsp?zoneCode="+zoneCode+"&dateTime="+dateTime+"&transmitName="+menuName+"清单["+zoneName+"]";
...
var bulidLineChart=function(map)
{
	...
	var cid=$("cid").value;
	dhx.showProgress("正在执行......");
    CmplGxbMonAction.bulidLineChart(map, {callback:function (res) {
    	        dhx.closeProgress();
...
{   
	...
	var cid1=$("cid1").value;
	dhx.showProgress("正在执行......");
    CmplGxbMonAction.bulidBarChart(map, {callback:function (res) {
    	dhx.closeProgress();
    	         if(res != null && res.XML != null){
