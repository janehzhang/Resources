...
var excuteInitData=function(){
    ...
    map.indexType   =$("indexType").value;
     ...
     ZtSatisfyVistListAction.getTableData(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
			    		         ...
			    		         if(tempColumn.indexOf('_')==-1)
				    		       {
			    		               if(tempColumn == '回访结果')
			    		               {
			    		            	 if(dataColumn[i].满意度 == "响应速度慢" || dataColumn[i].满意度 == "系统不稳定"|| dataColumn[i].满意度 == "办理失败"|| dataColumn[i].满意度 == "操作不方便"|| dataColumn[i].满意度 == "界面不美观"|| dataColumn[i].满意度 == "提示不清晰"|| dataColumn[i].满意度 == "其他")//不满意, 非常不满意 ,其它
			    		            	  {
			    		                    tableData +="<td><span style='float: left;'>"+dataColumn[i][tempColumn]+"</span>";
			    		                      tableData +="<img onclick=\"fillVisitResult('"+dataColumn[i].主键_ID+"','"+dataColumn[i].用户号码+"','"+dataColumn[i].统计_时间+"')\" border='0' style='cursor:pointer;float:right;' src='../../../../../images/btn-edit.gif' title='点击可编辑'/>";
			    		            	   tableData +="</td>"; 
			    		            	  }
			    		            	 else
			    		            	  {
...
var fillVisitResult=function(id,phone,date)
{
	//alert(id+"|"+phone.value)
    ...
    var url=base+"/portalCommon/module/procedure/visitDetail/szt/openVisitResult.jsp?id="+id+"&phone="+phone+"&date="+date+"&name="+el.innerHTML;
