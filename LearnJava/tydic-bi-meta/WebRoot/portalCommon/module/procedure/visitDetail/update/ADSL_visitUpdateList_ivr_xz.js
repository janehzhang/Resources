...
var excuteInitData=function(){
    ...
    // map.reasonId    =$("reasonId").value;
     ...
     ADSLVisitUpdateListAction.getTableDataIVRXZ(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
			    		         ...
			    		         if(tempColumn.indexOf('_')==-1)
				    		       {
			    		               if(tempColumn == '回访结果')
			    		               {			    		            	 
			    		            	 if(dataColumn[i].满意度 == "不满意" || dataColumn[i].满意度 == "报障响应不及时" || dataColumn[i].满意度 == "修障过程中未及时反馈"|| dataColumn[i].满意度 == "业务修复超时"|| dataColumn[i].满意度 == "服务态度不好"|| dataColumn[i].满意度 == "服务人员技术水平差"|| dataColumn[i].满意度 == "其他原因" )//不满意, 非常不满意 ,其它
			    		            	  {			    		        
