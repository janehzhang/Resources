...
var excuteInitData=function(){
    ...
    map.indexType   =$("indexType").value;
     ...
     ADSLVisitListAction.getTableData(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
			    		         ...
			    		         if(tempColumn.indexOf('_')==-1)
				    		       {
			    		               if(tempColumn == '回访结果')
			    		               {
			    		            	 if(dataColumn[i].满意度 == "未按时上门" || dataColumn[i].满意度 == "未按时开通"  || dataColumn[i].满意度 == "服务态度不好"|| dataColumn[i].满意度 == "服务人员技术水平差"|| dataColumn[i].满意度 == "其他原因")//不满意, 非常不满意 ,其它
			    		            	  {
