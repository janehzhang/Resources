...
var excuteInitData=function(){
     ...
     CustomerSatisfiedAction.getsmsDelayList(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
			    		         ...
			    		         if(tempColumn.indexOf('_')==-1)
				    		       {
			    		               if(tempColumn == '回访结果')
			    		               {
			    		            	 if(dataColumn[i].满意度 == "对套餐解释不清不满意" ||dataColumn[i].满意度 == "对业务办理差错不满意" || dataColumn[i].满意度 == "对其他不满意"  || dataColumn[i].满意度 == "其它")//不满意, 非常不满意 ,其它
			    		            	  {
