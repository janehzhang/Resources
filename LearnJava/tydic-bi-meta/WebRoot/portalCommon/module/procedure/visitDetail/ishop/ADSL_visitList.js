			    		         ...
			    		         if(tempColumn.indexOf('_')==-1)
				    		       {
			    		               if(tempColumn == '回访结果')
			    		               {
			    		            	 if(dataColumn[i].满意度 == "不满意")
			    		            	  {
			    		                    tableData +="<td><span style='float: left;'>"+dataColumn[i][tempColumn]+"</span>";
			    		                    tableData +="<img onclick=\"fillVisitResult('"+dataColumn[i].营业员_编号+"','"+dataColumn[i].首次回执打印流水+"','"+dataColumn[i].地市+"','"+dataColumn[i].统计_时间+"')\" border='0' style='cursor:pointer;float:right;' src='../../../../../images/btn-edit.gif' title='点击可编辑'/>";
			    		            	   tableData +="</td>"; 
			    		            	  }
			    		            	 else
			    		            	  {
...
var fillVisitResult=function(busiNo , custNo, areaName, createTime)
{
   var el = event.srcElement.previousSibling;
    ...
    var url=base+"/portalCommon/module/procedure/visitDetail/ishop/openVisitResult.jsp?busiNo="+busiNo+"&custNo="+custNo+"&areaName="+areaName+"&createTime="+createTime+"&name="+el.innerHTML;
