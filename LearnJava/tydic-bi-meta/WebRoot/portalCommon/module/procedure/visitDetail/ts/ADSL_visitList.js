...
var excuteInitData=function(){
    ...
    // map.reasonId    =$("reasonId").value;
     ...
     ADSLVisitUpdateListAction.getTsTableData(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
			    		         ...
			    		         if(tempColumn.indexOf('_')==-1)
				    		       {
			    		               if(tempColumn == '回访结果')
			    		               {
			    		            	 if(dataColumn[i].满意度 == "不满意" || dataColumn[i].满意度 == "非常不满意"  || dataColumn[i].满意度 == "其它")//不满意, 非常不满意 ,其它
			    		            	  {
			    		                    tableData +="<td><span style='float: left;'>"+dataColumn[i][tempColumn]+"</span>";
			    		                      tableData +="<img onclick=\"fillVisitResult('"+dataColumn[i].工单流水号+"','"+dataColumn[i].环节编号+"','"+dataColumn[i].回访方式+"','"+dataColumn[i].统计_时间+"')\" border='0' style='cursor:pointer;float:right;' src='../../../../../images/btn-edit.gif' title='点击可编辑'/>";
			    		            	   tableData +="</td>"; 
			    		            	  }
			    		            	 else
			    		            	  {
...
var fillVisitResult=function(busiNo,hjNo,hfType,countTime)
{
	//alert(busiNo+'|'+hjNo+'|'+hfType+'|'+countTime);
    ...
    var url=base+"/portalCommon/module/procedure/visitDetail/ts/openVisitResult.jsp?busiNo="+busiNo+"&hjNo="+hjNo+"&hfType="+hfType+"&countTime="+countTime+"&name="+el.innerHTML;
...
var expExcel=function(){
    ...
    // map.reasonId    =$("reasonId").value;
     map.satisType   =$("satisType").value;
    	 map.pageCount=  $("totalCount").value;    //每页显示多少条数 
    	// alert(map.pageCount)
     map.currPageNum=page.currPageNum;//当前第几页
     dhx.showProgress("正在执行......");
     ADSLVisitUpdateListAction.getTsTableData(map, function (res) {
           ...
           var  excelHeader="";
	var  tableHead="<tr>";
	       for(var i=0;i<data.headColumn.length;i++)
	       {
	          if(data.headColumn[i].indexOf('_')==-1)//字段 有 "-"  表示不展示
	    	  {
	           tableHead +="<td nowrap bgcolor='#cde5fd'><span class='title'>"+data.headColumn[i]+"</span></td>";  
	           excelHeader += data.headColumn[i]+",";
	          }
	       }
	     tableHead+="</tr>";
	    tableStr+=tableHead;
	    ...
	    var headColumn=data.headColumn;
	      if(dataColumn&&dataColumn.length>0)
		    {  
		       for(var i=0;i<dataColumn.length;i++)
		       {
