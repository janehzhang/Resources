...
var excuteInitData=function(){
    ...
    // map.reasonId    =$("reasonId").value;
     ...
     ADSLVisitUpdateListAction.getVipTableData(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
			    		         ...
			    		         if(tempColumn.indexOf('_')==-1)
				    		       {
			    		               if(tempColumn == '回访结果')
			    		               {
			    		            	 if(dataColumn[i].满意度 == "服务态度不满意" || dataColumn[i].满意度 == "业务水平不满意"  || dataColumn[i].满意度 == "等候时间不满意"|| dataColumn[i].满意度 == "办理效率不满意"|| dataColumn[i].满意度 == "营业环境不满意")//不满意, 非常不满意 ,其它
			    		            	  {
			    		                    tableData +="<td><span style='float: left;'>"+dataColumn[i][tempColumn]+"</span>";
			    		                      tableData +="<img onclick=\"fillVisitResult('"+dataColumn[i].营业员_编号+"','"+dataColumn[i].手机号码+"','"+dataColumn[i].地市+"','"+dataColumn[i].统计_时间+"')\" border='0' style='cursor:pointer;float:right;' src='../../../../../images/btn-edit.gif' title='点击可编辑'/>";
			    		            	   tableData +="</td>"; 
			    		            	  }
			    		            	 else
			    		            	  {
			    		               ...
			    		               {
			    		            	    tableData +="<td nowrap align='left' title='"+dataColumn[i][tempColumn]+"'>"+replaceChar(dataColumn[i][tempColumn])+"</td>";    
			    		               }
			    		               if(tempColumn !="统计_时间"){
...
var fillVisitResult=function(busiNo , custNo, areaName, createTime)
{
	//alert(busiNo +"|"+ custNo+"|"+areaName+"|"+createTime);
   var el = event.srcElement.previousSibling;
    ...
    var url=base+"/portalCommon/module/procedure/visitDetail/hf/openVisitResult.jsp?busiNo="+busiNo+"&custNo="+custNo+"&areaName="+areaName+"&createTime="+createTime+"&name="+el.innerHTML;
