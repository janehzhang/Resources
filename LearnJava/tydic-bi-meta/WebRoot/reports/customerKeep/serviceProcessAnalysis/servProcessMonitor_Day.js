...
var titileInfo=$("indId").options[$("indId").selectedIndex].text+"->当日值";
var titileInfo1=$("indId").options[$("indId").selectedIndex].text+"->本期累计";
...
var excuteInitData=function(changeCode){
     ...
     map.indId=$('indId').value==""?"1":$('indId').value;

     map.changeCode=changeCode;
     
     map.field="CURRENT_VALUE";
     map.rptIndex="servProcessMonitorDay";
     zoneCode    =$("zoneCode").value==""?userInfo['localCode']:$("zoneCode").value;
     
     map.reportId="30012";
     map.parameters=parameters; 
     map.values= $("dateTime").value.replaceAll("-", "")+"$"+zoneCode+"$"+$('prodTypeCode').value;
...
{	
	var url="/reports/customerKeep/serviceProcessAnalysis/servProcessMonitor_Day.jsp?dateTime="+$("dateTime").value+"&zoneCode="
	+zoneCode+"&prodTypeCode="+$('prodTypeCode').value+"&indId="+$('indId').value+"&menuId="+menuId;
	return parent.openTreeTab("124026"+zoneCode,"服务过程监控类指标日监测报表"+"["+zoneName+"]",base+url,'top');	
}
/**
 * 
 * @param myObject
...
function changeCol(obj){
	$("titleInfo1").innerHTML=$("indId").options[$("indId").selectedIndex].text+"->当日值";
	$("titleInfo2").innerHTML=$("indId").options[$("indId").selectedIndex].text+"->本期累计"; 
...
function exportExcel(){
	    ...
	    var  prodType=$("prodTypeCode").options[$("prodTypeCode").selectedIndex].text;//业务类型
	    var queryCond="日期："+dateTime+"    区域："+zone+"    业务类型："+prodType;
