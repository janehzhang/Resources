...
var excuteInitData=function(){
    ...
    map.rptIndex="serviceReActiveMonP";
...
var  excelHeader="";
      for(var i=0;i<headColumn.length;i++)
      {
       if(headColumn[i].indexOf('_')==-1)//字段 有 "-"  表示不展示
	    {
    	  tableHead="<td bgcolor='#cde5fd' style='width:60px' rowspan='2'><span class='title'>大区</span></td>"
			    		     ...
			    		     for(var j=0;j<headColumn.length;j++)
			    		      {
			    		               ...
			    		               if(tempColumn.indexOf('_')==-1)
					    		       {
				    		        	 ...
				    		        	 }else if(tempColumn == '移动复机工单总数' )
				    		        	 {
						    		         tableData +="<td nowrap onclick=\"openMenuHref('"+dataColumn[i].分公司_ID+"','"+dataColumn[i].分公司+"','"+dataColumn[i].日期+"','30')\" class='unl'>"+dataColumn[i][tempColumn]+"</td>";	 
				    		        	 }else if(tempColumn == '宽带复机工单总数' )
				    		        	 {
						    		         tableData +="<td nowrap onclick=\"openMenuHref('"+dataColumn[i].分公司_ID+"','"+dataColumn[i].分公司+"','"+dataColumn[i].日期+"','40')\" class='unl'>"+dataColumn[i][tempColumn]+"</td>";	 
				    		        	 }else{
...
{	
	...
	var url="/reports/customerKeep/serviceProcessAnalysis/serviceReActive_monP.jsp?zoneCode="+zoneCode+"&dateTime="+dateTime+"&selectCol="+selectCol+"&menuId="+menuId;
	return parent.openTreeTab("116092"+zoneCode, "停机复开及时率月报"+"["+zoneName+"]", base+url, 'top');
}
//第一列合并
...
function exportExcel(){
		    var  dateTime=$("dateTime").value;//时间
 ...
 * 
 */
var openMenuHref=function(zoneCode,regionName,dateTime,prodId){
	var year=dateTime.substring(0,4);
	var mon=dateTime.substring(4,6);
	...
	var url="/reports/customerKeep/serviceProcessAnalysis/serviceReActiveDetail.jsp?zoneCode="+zoneCode+"&startDate="+time+"&endDate="+time+"&prodId="+prodId+"&menuId="+menuId;
	return parent.openTreeTab("143003"+zoneCode+prodId,prodName+"停复机清单["+regionName+"]",base+url,'top');
}
dhx.ready(indexInit);