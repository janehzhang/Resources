...
<%
    ...
    HSSFSheet sheet = workBook.createSheet("移动和宽带用户停机复开各环节平均时间月报表");
   ...
   List<Map<String, Object>> cellList=dao.getServiceAvgSecends_Mon(queryData);
  if(cellList != null && cellList.size()>0 ){
	   for (Map<String, Object> key : cellList) {
	          data.add(key.get("PAR_REGION_NAME").toString());
	          data.add(key.get("REGION_NAME").toString());
	          data.add(key.get("YD_CBS_COSTTIME").toString());
	          data.add(key.get("YD_CRM_COSTTIME").toString());
	          data.add(key.get("YD_SPS_COSTTIME").toString());
	          data.add(key.get("YD_TOTAL_AVG_COSTTIME").toString());
	          data.add(key.get("KD_CBS_COSTTIME").toString());
	          data.add(key.get("KD_CRM_COSTTIME").toString());
	          data.add(key.get("KD_SPS_COSTTIME").toString());
	          data.add(key.get("KD_TOTAL_AVG_COSTTIME").toString());
	        excel.createRow(data,row_num++,col_num,excel.getBodyStyle());
	    	data.clear();
	   }
	 }
