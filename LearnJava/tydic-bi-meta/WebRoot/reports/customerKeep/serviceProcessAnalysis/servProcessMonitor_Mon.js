...
var titileInfo=$("indId").options[$("indId").selectedIndex].text+"->当月值";
...
var excuteInitData=function(changeCode){
     ...
     map.rptIndex="servProcessMonitor";
...
{	
	var url="/reports/customerKeep/serviceProcessAnalysis/servProcessMonitor_Mon.jsp?dateTime="+$("dateTime").value+"&zoneCode="
	+zoneCode+"&prodTypeCode="+$('prodTypeCode').value+"&indId="+$('indId').value+"&menuId="+menuId;
	return parent.openTreeTab("124025"+zoneCode,"服务过程监控类指标月监测报表"+"["+zoneName+"]",base+url,'top');	
}
/**
 * 
 * @param myObject
...
function changeCol(obj){
	$("titleInfo1").innerHTML=$("indId").options[$("indId").selectedIndex].text+"->当月值";
...
function exportExcel(){
	    ...
	    var queryCond="月份："+dateTime+"    区域："+zone+"    业务类型："+prodType;
