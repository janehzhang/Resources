		    		     ...
		    		     for(var j=0;j<headColumn.length;j++)
		    		      {
		    		               ...
		    		               if(tempColumn == '分公司'&&i!=0)
				    		    	 ...
				    		    	 else if(tempColumn.indexOf('_')==-1)
				    		       {
				    		          if(tempColumn == '回访量')
				    		          {
				    		          ...
				    		          else
				    		          {
				    		             ...
				    		             {
				    		             tableData +="<td nowrap align='center' onclick=\"lookMenuHref('"+dataColumn[i].分公司+"','"+dataColumn[i].分公司_编码+"','"+tempColumn+"','3')\" class='unl'>"+dataColumn[i][tempColumn]+"</td>";   	
				    		             }
				    		            else
				    		             {
 ...
 * 
 */
var  lookMenuHref=function(zoneName,zoneCode,indexName,indexId){
	   var dateTime=$("dateTime").value;
	   var url="/reports/customerSatisfied/broadbandNew/noSatisfiedReason_Week.jsp?zoneCode="+zoneCode+"&dateTime="+dateTime;
	   return parent.openTreeTab("122006"+zoneCode, "宽带新装不满意原因TOP周报"+"["+zoneName+"]", base+url, 'top');
}
...
var openMenuHref=function(zoneName,zoneCode,indexName,indexId){
	 ...
	 var url="/portalCommon/module/procedure/visitDetail/ADSL_visitList.jsp?zoneCode="+zoneCode+
	   "&startTime="+startTime+"&endTime="+endTime+"&indexId="+indexId; 
	 return parent.openTreeTab("121001"+zoneCode+indexId, "宽带新装即时回访清单"+"["+zoneName+indexName+"]", base+url, 'top');
}
//钻取
...
{	
	...
	var url="/reports/customerSatisfied/broadbandNew/serviceSatisfied_Week.jsp?zoneCode="+zoneCode+"&dateTime="+dateTime+"&selectCol="+selectCol+"&menuId="+menuId;
	return parent.openTreeTab("122001"+zoneCode, "宽带新装满意率周报"+"["+zoneName+"]", base+url, 'top');
}
//通过列改变图形
