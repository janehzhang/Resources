...
dwrCaller.addAction("querySubZoneCode",function(afterCall,param){
    ...
    ZoneAction.querySubZoneCode1(param.id,function(data){
...
function showdiv(obj,event,zoneName,zoneCode){
	...
	var menuIndex="";
	var height="";
	if(vTypeId=='0'||vTypeId=='1'||vTypeId=='2'){
		menuIndex="menu";
		height="80";
	}else if(vTypeId=='5'){
		menuIndex="menu1";
...
var queryData=function(){
    ...
    CustomerSatisfiedAction.getCustomerSatisfiedDetails_Sum_Dg(map, function (res) {
    	$('chartdiv1').innerHTML='';
    	$('chartdiv2').innerHTML='';
   	               dhx.closeProgress();
				        if (res == null) {
				    		     ...
				    		     for(var j=0;j<headColumn.length;j++)
				    		      {
				    		               ...
				    		               if(tempColumn.indexOf('_')==-1)
						    		       {
				    		            	   ...
				    		            	   }else if(tempColumn=='不满意'||tempColumn=='不满意量'){
...
var  lookMenuHref=function(zoneName,zoneCode,vTypeId){
	   ...
	   var url="/reports/customerSatisfied/noSatisfied/noSatisfiedReason_Sum.jsp?zoneCode="+zoneCode+"&dateTime="+time+"&vTypeId="+vTypeId;
...
var  lookMenuHref1=function(zoneName,zoneCode,vTypeId){
	   ...
	   var url="/reports/customerSatisfied/noSatisfied/noSatisfiedReason_Sum.jsp?zoneCode="+zoneCode+"&dateTime1="+time+"&vTypeId="+vTypeId;
	   return parent.openTreeTab("133005"+zoneCode, "不满意原因排名"+"["+zoneName+"]", base+url, 'top');
}
//日
var  lookMenuHref2=function(zoneName,zoneCode,vTypeId){
	   ...
	   var url="/reports/customerSatisfied/noSatisfied/noSatisfiedReason_Sum.jsp?zoneCode="+zoneCode+"&startDate="+startTime+"&endDate="+endTime+"&vTypeId="+vTypeId;
	   return parent.openTreeTab("133005"+zoneCode, "不满意原因排名"+"["+zoneName+"]", base+url, 'top');
}
//查看满意度量值
//周
var  goMenuHref=function(zoneName,zoneCode,vTypeId,indexId){
	   ...
	   endTime =endTime.substr(0,4)+"-"+endTime.substr(4,2)+"-"+endTime.substr(6,2) ;
		 ...
		 return parent.openTreeTab("124017"+zoneCode+indexId+vTypeId, zoneName+"即时回访清单", base+url, 'top');
}
//月
...
var returnMenuHref=function(zoneName,zoneCode,reasonId){
	 ...
	 return parent.openTreeTab("124017"+zoneCode+vTypeId+reasonId, zoneName+"即时回访清单", base+url, 'top');
}
//月
...
function buildChart(map){
	dhx.showProgress("正在执行......");
	CustomerSatisfiedAction.getCustomerSatisfiedDetails_GraphSum_Dg(map, {callback:function (res) {
		dhx.closeProgress();
    	         if(res != null){
                        buildBLChart(res);
		         }
