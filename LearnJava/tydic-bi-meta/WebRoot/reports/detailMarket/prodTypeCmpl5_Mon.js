...
var excuteInitData=function(){
		 ...
		 map.zoneCode=$('zoneCode').value==""?"0000":$("zoneCode").value;
 	     ...
 	     DetailMarketAction.queryDetailMarket1_pg(map, function (res) {
	     	   dhx.closeProgress();
	 			 if (res == null) {
	 				   dhx.alert("读取数据发生错误,请稍后重试，或者联系系统管理员!");
	 				     return;
	 			 }else{
	 				chart1=new FusionCharts(base+"/js/Charts/Pie3D.swf", "ChartId1_"+cid1, "100%", "300", "0", "0");
	 	 		    chart2=new FusionCharts(base+"/js/Charts/Pie3D.swf", "ChartId2_"+cid, "100%", "300", "0", "0");
	 	 		    chart1.setDataXML(res.pieChartMap1);
...
function tableHeader(headColumn){
	var tableHead="<tr>"
		+"<td bgcolor='#cde5fd' rowspan='2'><span class='title'>产品类别</span></td>"
		+"<td bgcolor='#cde5fd' rowspan='2'><span class='title'>服务类别</span></td>"
		+"<td bgcolor='#cde5fd' colspan='6'><span class='title'>本地投诉量</span></td>"
		+"<td bgcolor='#cde5fd' colspan='6'><span class='title'>本地投诉率（‰）</span></td>"
		+"</tr>"
		+"<tr>"
...
function buildChart(map){
	dhx.showProgress("正在执行......");
	DetailMarketAction.getDetailMarket1_PieChart(map, {callback:function (res) {
		dhx.closeProgress();
    	         if(res != null){
                        buildBLChart(res);
		         }
...
function  buildBLChart(map){
	    ...
	    chart1=new FusionCharts(base+"/js/Charts/Pie3D.swf", "ChartId1_"+cid1, "100%", "300", "0", "0");
	    chart2=new FusionCharts(base+"/js/Charts/Pie3D.swf", "ChartId2_"+cid, "100%", "300", "0", "0");
	    chart1.setDataXML(map.pieChartMap1);
        chart2.setDataXML(map.pieChartMap2);
        chart1.render("chartdiv1");
        chart2.render("chartdiv2"); 
   }
 //导出
...
function exportExcel(){
		    ...
		    var queryCond="月份："+startDate+"    区域："+zone; 
	   	    ...
	   	    var url = getBasePath()+"/portalCommon/module/procedure/impExcel/implExcel_twoPieChartHeader.jsp";
