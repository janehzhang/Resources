...
var excuteInitData=function(){
	...
	var cid1=$("cid1").value;
     ...
     CmplIndexAction.getCmplDayData(map, function (res) {
     	$('chartdiv1').innerHTML='';
     	$('chartdiv2').innerHTML='';
    	               dhx.closeProgress();
 				        if (res == null) {
...
var drillArea=function(zoneName,zoneCode)
{
	...
	var prodTypeCode = $('prodTypeCode').value==''?"-1":$('prodTypeCode').value;
	var cmplBusiTypeCode =   $('cmplBusiTypeCode').value==''?"1":$('cmplBusiTypeCode').value;
	var url="/reports/cmpl/day/allBusiCmpl_Day.jsp?dataType="+$('dataType').value+"&startDate="+startDate+"&endDate="+endDate
	+"&indexId="+indexType+"&zoneCode="+zoneCode+"&prodTypeCode="+prodTypeCode+"&cmplBusiTypeCode="+cmplBusiTypeCode+"&menuId="+menuId;
	var ReportName = null;
	if($('dataType').value=="0")
		ReportName = "本地抱怨率日报表";
	if($('dataType').value=="1")
		ReportName = "本地抱怨率日报表_营服中心"
	return parent.openTreeTab("123006"+zoneCode,ReportName+"["+zoneName+"]",base+url,'top');	
}
//抱怨量钻取
...
function exportExcel(){
		...
		var  cmplBusiTypeCode=$("cmplBusiType").value;//文本框
