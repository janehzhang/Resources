			    		        	   ...
			    		        	   tableData +="<td nowrap onclick=\"drillArea('"+dataColumn[i].第一级+"','"+dataColumn[i].第一级_ID+"')\" class='unl'>"+dataColumn[i][tempColumn]+"</td>";	 
			    		        		 ...
			    		        		 if(userCodeData.indexOf("]"+zoneCode+"]")==-1){
				    		        		 ...
				    		        		 if(dataColumn[i].第三级_ID=='-1001'){//合计
				    		        			 tableData +="<td onClick=\"drillList('"+$("cmplBusinessType").value+"','"+$("cmplBusinessType").value+"')\"  class='unl'>"+dataColumn[i][tempColumn]+"</td>";
...
{	
	...
	var ind=$("ind").value;
	var url="/reports/cmpl/mon/nativeCmplPhe_Mon.jsp?zoneCode="+zoneCode+"&dateTime="+dateTime+"&ind="+ind+"&cmplBusinessType="+cmplTypeId+"&prodTypeCode="+prodTypeCode+"&menuId="+menuId;
	return parent.openTreeTab("122033"+zoneCode, "投诉现象分析月报表"+"["+cmplType+"]", base+url, 'top');
}
...
function exportExcel(){
	    ...
	    var queryCond="月份："+dateTime+"    区域："+zone+"    投诉现象: "+cmplBusinessType +"    业务类型: "+prodType ;  
