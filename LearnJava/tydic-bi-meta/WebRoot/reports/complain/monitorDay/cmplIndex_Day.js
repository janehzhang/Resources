    ...
    loadParam.setParamConfig([
        {
        	...
        	formData.field="CURRENT_NUM";
        	formData.rptIndex="cmplIndexDay"; 
...
var excuteInitData=function(){
	dwrCaller.executeAction("getCmplIndex",function(data){
		...
		var tableStr="<table width='970' id='mytbl' border='0' cellpadding='0' cellspacing='0' class='tab_01'>"
			  +"<tr>"
			   +"<td bgcolor='#cde5fd' style='width:60px'><strong>大区</strong></td>"
			    ...
			    +"<td bgcolor='#cde5fd' style='width:60px'>当日值</td>"
			    +"<td bgcolor='#cde5fd' style='width:60px'>本期累计</td>"
			    +"<td bgcolor='#cde5fd' style='width:60px'>上月同期累计</td>"
			    +"<td bgcolor='#cde5fd' style='width:80px'>环比</td>"
			    +"<td bgcolor='#cde5fd' style='width:60px'>同比</td>"
			    +"<td bgcolor='#cde5fd' style='width:60px'>本期平均</td>"
			 +"</tr>";
       ...
       for(var i=0;i<data.list.length;i++){
        ...
        if(data.list[i].IND_ID=="3"||data.list[i].IND_ID=="4"){
        	 tableStr2="<td onClick=\"drillList('"+data.list[i].REGION_ID+"','"+data.list[i].IND_ID+"','"+data.list[i].IND_NAME+"','"+data.list[i].REGION_NAME+"')\"  class='unl'>"+data.list[i].CURRENT_NUM+"</td>";
        }else{
        	 tableStr2="<td>"+data.list[i].CURRENT_NUM+"</td>";
        }
         tableStr3="<td>"+data.list[i].TOTAL_NUM+"</td>"
             +"<td>"+data.list[i].TOTAL_NUM_LAST+"</td>"
             +"<td>"+data.list[i].HB_NUM+"%"+"</td>"
             +"<td>"+data.list[i].TB_NUM+"%"+"</td>"
             +"<td>"+data.list[i].AVG_NUM+"</td>"
		    +"</tr>";
         tableStr+=tableStr1+tableStr2+tableStr3
         }
		       tableStr +="</table>"; 
...
var drillList=function(zoneCode,indId,indName,regionName){
	var url="/reports/portal/cmpl_list_mon.jsp?dateTime="+$('dateTime').value+"&zoneCode="
	+zoneCode+"&prodTypeCode="+$('prodTypeCode').value+"&indId="+indId;
	return parent.openTreeTab("116087"+indId+zoneCode,regionName+"—>"+indName+"清单",base+url,'top');
}
dhx.ready(indexInit);