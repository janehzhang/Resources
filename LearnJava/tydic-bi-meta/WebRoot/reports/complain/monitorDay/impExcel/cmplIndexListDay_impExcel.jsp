...
<%
    ...
    HSSFSheet sheet = workBook.createSheet("投诉类指标日监测报表");
   ...
   List<Map<String, Object>> cellList=dao.getCmplIndex_Day(queryData);
  if(cellList != null && cellList.size()>0 ){          
	   for (Map<String, Object> key : cellList) {
	          ...
	          data.add(key.get("CURRENT_NUM").toString());
	          data.add(key.get("TOTAL_NUM").toString());
	          data.add(key.get("TOTAL_NUM_LAST").toString());
	          data.add(key.get("HB_NUM").toString());
	          data.add(key.get("TB_NUM").toString());
	          data.add(key.get("AVG_NUM").toString());
