...
dwrCaller.addAutoAction("loadChannelSerTree","NewTwoChannelAction.queryChannelNewServById");
...
dwrCaller.addAction("querySubChannelServId",function(afterCall,param){
    ...
    NewTwoChannelAction.querySubChannelNewServId(param.id,function(data){
        data=tempCovert.convert(data);
        afterCall(data);
    })
});
...
var excuteInitData=function(){
     ...
     map.secondId   =$("secondId").value;
     map.pageCount=  page.pageCount;    //每页显示多少条数
     map.currPageNum=page.currPageNum;//当前第几页
     
     dhx.showProgress("正在执行......");
     NewTwoChannelAction.getChannelNewIndexTableData(map, function (res) {
    	               dhx.closeProgress();
				        if (res == null) {
