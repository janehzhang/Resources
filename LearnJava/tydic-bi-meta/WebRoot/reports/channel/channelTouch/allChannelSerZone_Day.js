...
/** channel Tree head **/
	...
	dwrCaller.addAction("querySubChannelTypePart",function(afterCall,param){
	    ...
	    ChannelTypeAction.querySubChannelTypeZone(param.id,function(data){
	        data=tempCovert.convert(data);
	        afterCall(data);
	    })
	});
...
var excuteInitData=function(){
	...
	$("titleInfo1").innerHTML=titileInfo;
		...
		var cid1=$("cid1").value;
	     ...
	     AllBusinessAction_New.getAllChannelSerZone_Pg(map, function (res) {
	     	$('chartdiv1').innerHTML='';
	     	$('chartdiv2').innerHTML='';
	    	               dhx.closeProgress();
	 				        if (res == null) {
...
{	
	...
	var selectCol=$("selectCol").value;
	var channelTypeCode=$('channelTypeCode').value==""?'1':$("channelTypeCode").value;
	var url="/reports/channel/channelTouch/allChannelSerZone_Day.jsp?zoneCode="+zoneCode+"&startDate="+startDate+"&endDate="+endDate+"&channelTypeCode="
	+channelTypeCode+"&selectCol="+selectCol+"&menuId="+menuId;
	return parent.openTreeTab("155012"+zoneCode+channelTypeCode, "全渠道服务区域日报"+"["+zoneName+"]", base+url, 'top');
...
var lookCity = function (obj){
        ...
        tZoneCode=$("zoneCode").value;
     	if(tZoneCode!='0000') {
		  ...
		  dhx.showProgress("正在执行......");
		  AllBusinessAction_New.getAllChannelSerZone_BarChart(map, {callback:function (res) {
			  dhx.closeProgress();
 	         if(res != null){
                     build21Chart(res);
		         }
...
function  build21Chart(map){
	   ...
	   chart.setDataXML(map.barChartMap);
...
function buildChart(map){
	dhx.showProgress("正在执行......");
	AllBusinessAction_New.getAllChannelSerZone_Chart(map, {callback:function (res) {
		dhx.closeProgress();
    	         if(res != null){
                        buildBLChart(res);
		         }
