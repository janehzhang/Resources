...
var titileInfo1=$("indId").options[$("indId").selectedIndex].text+"->当日值";
...
var excuteInitData=function(){
     ...
     map.rptIndex="channelSerMonitorDay";
     ...
     AllBusinessAction.getChannelSer_pg(map, function (res) {
     	$('chartdiv1').innerHTML='';
     	$('chartdiv2').innerHTML='';
    	               dhx.closeProgress();
 				        if (res == null) {
...
{	
	var url="/reports/channel/dayMonitor/channelSerMonitor_Day.jsp?dateTime="+$("dateTime").value+"&zoneCode="
	+zoneCode+"&indId="+$('indId').value+"&menuId="+menuId;
	return parent.openTreeTab("125008"+zoneCode,"渠道服务类指标日监测报表"+"["+zoneName+"]",base+url,'top');	
}
/**
 * 
 * @param myObject
...
var lookCity = function (obj){
     ...
     map.values= $("dateTime").value.replaceAll("-", "")+"$"+zoneCode;//+"$"+$('prodTypeCode').value
    	...
    	dhx.showProgress("正在执行......");
	AllBusinessAction.loadSet21AreaChart_ChannelSer(map, {callback:function (res) {
		dhx.closeProgress();
    	         if(res != null){
                        buildBChart(res);
		         }
...
function buildChart(map){
	dhx.showProgress("正在执行......");
	AllBusinessAction.getChannelSerChange(map, {callback:function (res) {
		dhx.closeProgress();
    	         if(res != null){
                        buildBLChart(res);
		         }
...
function  buildBChart(map){
		$("cid1").value=Math.random();
		var cid1=$("cid1").value;
	   $("chartdiv1").innerHTML="";
	   chart1=new FusionCharts(base+"/js/Charts/MSColumn2D.swf", "ChartId1_"+cid1, "100%", "200", "0", "0");
	   chart1.setDataXML(map.XML);
       chart1.render("chartdiv1");
   }
