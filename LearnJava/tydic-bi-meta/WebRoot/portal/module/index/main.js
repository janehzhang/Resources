...
var openTab = function(menuId, menuData,isRefresh) {
    if (!mainTabbar) {
        ...
        mainTabbar.setSize(tabWidth,tabHeight);
        //Tab设置
        ...
        div.innerHTML = "您好," + user.userNamecn;
    ...
    if (tabCount <= maxTabCont) {
        ...
        if (menuData.target == "top") {//顶层弹出Tab
            if (!menuTabs[tabId]) {
                ...
                mainTabbar.addTab(tabId, menuData.menuName, width);
                tabCount++;
                //判断是否有子节点，有子节点需加载树。
 ...
 * @param count
 */
var refreshOnlineCount=function(count){
    var arg=arguments;
    if($("online_user_num")){
        $("online_user_num").innerHTML=count<1?1:count;
    }
//		else{
//        setTimeout(function(){
//            arg.callee.call(this,count);
//        },50)
//    }
}
...
var globalInit = function(frameWin, menuId) {
    ...
    PortalCtrlr.getDuty(function(res){
