...
public class ThirdViewImpl implements MultViewInterface{
	...
	public String createJSON(String jsonModel, List<Map<String, Object>> list,
		...
		String value = " {\"value\":\"{category_value}\",\"color\":\"{category_color}\",\"link\":\"JavaScript:multDimenAnaly.getSubBusiData({parent}, {level});\"} ";
		...
		if(result!=null&&!result.equalsIgnoreCase("")){
			jsonModel = jsonModel.replace("{category_label_content}", categoryBuffer.toString().substring(0, categoryBuffer.toString().length()-1))
					...
					.replace("{xAxisName}", entity.getW3_x_name())
					.replace("{yAxisName}", entity.getW3_y_name());
		}else {
	...
	public String createRateJSON(String jsonModel, List<Map<String, Object>> list,
		...
		if(!list.isEmpty()){
			...
			jsonModel = jsonModel.replace("{xAxisName}", entity.getW3_x_name()).replace("{yAxisName}", entity.getW3_y_name());
		}else {
	...
	public String createRateXML(String xmlModel, List<Map<String, Object>> list,
		...
		for(Map<String, Object> map : list){
			...
			dataBuffer.append(dataSet.replace("{x}", String.valueOf(i)).replace("{y}", ratio).replace("{text}", name + "," + _value + "%").replace("{color}", ColorBox.getColor(1)));
