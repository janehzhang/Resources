...
public class SecondViewImpl implements MultViewInterface{
	...
	public String createJSON(String jsonModel, List<Map<String, Object>> list,
		...
		String value = " {\"value\":\"{category_value}\",\"color\":\"{category_color}\",\"link\":\"JavaScript:multDimenAnaly.getSubProdData({parent}, {level});\"} ";
		...
		for(Map<String, Object> map : list){
			...
			valueBuffer.append(value.replace("{category_value}", _value).replace("{category_color}", ColorBox.getColor(i)).replace("{parent}", code).replace("{level}", entity.getDr_level())+",");
			if((i+1)%2!=0){
				if(i==0){
		...
		if(result!=null&&!result.equalsIgnoreCase("")){
			jsonModel = jsonModel.replace("{category_label_content}", categoryBuffer.toString().substring(0, categoryBuffer.toString().length()-1))
					...
					.replace("{xAxisName}", entity.getW2_x_name())
					.replace("{yAxisName}", entity.getW2_y_name());
		}else {
	...
	public String createRateJSON(String jsonModel, List<Map<String, Object>> list,
		...
		for(Map<String, Object> map : list){
			...
			if(flag==1){
				dataBuffer.append(dataSet.replace("{x}", String.valueOf(i)).replace("{y}", ratio)
						.replace("{text}", name + "," + _value + "%")
		...
		if(!list.isEmpty()){
			...
			jsonModel = jsonModel.replace("{xAxisName}", entity.getW2_x_name()).replace("{yAxisName}", entity.getW2_y_name());
		}else {
	...
	public String createRateXML(String xmlModel, List<Map<String, Object>> list,
		...
		FirstViewEntity entity = (FirstViewEntity)obj;
		StringBuffer dataBuffer = new StringBuffer();
		String dataSet = "<set x=\"{x}\" y=\"{y}\" toolText=\"{text}\" color=\"{color}\" />";
		int i = 4;
		for(Map<String, Object> map : list){
			String _value = map.get("COMPLAIN_NUM").toString();
			String name = map.get("NAME").toString();
			...
			dataBuffer.append(dataSet.replace("{x}", String.valueOf(i)).replace("{y}", ratio).replace("{text}", name + "," + _value + "%").replace("{color}", tag_color));
		...
		if(!list.isEmpty()){
			...
			xmlModel = xmlModel.replace("{xAxisName}", entity.getW2_x_name()).replace("{yAxisName}", entity.getW2_y_name());
		}else {
