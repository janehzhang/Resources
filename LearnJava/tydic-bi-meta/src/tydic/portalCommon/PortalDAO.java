...
public class PortalDAO extends MetaBaseDAO {
    ...
    public static final String sourceName = "config2";    //数据源使用config2
    ...
    public Object[] getAreaName() {
        ...
        String sql = "SELECT DISTINCT T.ZONE_CODE,T.ZONE_NAME,T.DIM_LEVEL FROM META_DIM_ZONE T WHERE T.DIM_LEVEL<=3 ";
    ...
    public List<Map<String, Object>> getChart1(String type, String dragon_channel_type, String bt_code, String localCode, String minTime, String maxTime) throws Exception {
        ...
        if (type.toString().endsWith("1")) {
            isArive = true;
        }
        StringBuffer sqlBuf = new StringBuffer();
        List params = new ArrayList();
        if (isArive) {
            sqlBuf.append("select DATE_NO DATENO, sum(cn) Value2 " +
                    "  from (select UNIQUE DATE_NO, " +
                    "               sum(t.term_sum) over(order by DATE_NO) cn, " +
                    "               dense_rank() over(order by DATE_NO) rn " +
                    "          from DM_MATERIAL_SERV_D T " +
                    "         WHERE t.if_iphone=1 ");
            if (minTime != null && !"".equals(minTime)) {
            ...
            if (bt_code != null && !"".endsWith(bt_code)) {
                sqlBuf.append("and t.bt_code=? ");
                params.add(bt_code);
            }
            sqlBuf.append(") start with rn >=1 " +
                    "connect by rn - 1 = rn " +
                    " group by DATE_NO ");
        } else {
