 ...
 */
public class CmplSpanMonDAO extends MetaBaseDAO{
	...
	public Map<String, Object> getTableData(Map<String, Object> map) {
		...
		String storeName = ConstantStoreProc.RPT_CMPL_SPAN_MON;
		Object[] params = {
...
/*		sql.append("select sum(num1 + num3 + num6 + num7 + num9 + num10) 越级投诉量 " +
	    ...
	    sql.append("select a.yj_num  越级投诉量,");
		sql.append(" a.user_num 上月计费用户数,");
		sql.append(" to_char(DECODE(a.user_num,0,0,round((a.yj_num/a.user_num )*1000000,2)),'FM99990.00') 越级投诉率");
		sql.append(" from (select a.REGION_TREECODE,");
		sql.append(" yj_num,");
		sql.append(" b.user_num user_num");
		sql.append(" from (select t.treecode REGION_TREECODE, sum(num1+num3+num6+num7+num9+num10) yj_num");
...
/*    	sb.append("select sum(num1 + num3 + num6 + num7 + num9 + num10) 越级投诉量 " +
				   " FROM cs_span_cmpl a where a.month_id = '"+ dateTime +"' " +
				    " and a.region_treecode like  getTreeCode('"+code+"') || '%' ");*/
    	
	    String lastTime=getLastMon(dateTime);
	    sb.append("select a.yj_num  越级投诉量,");
		sb.append(" a.user_num 上月计费用户数,");
		sb.append(" to_char(DECODE(a.user_num,0,0,round((a.yj_num/a.user_num )*1000000,2)),'FM99990.00') 越级投诉率");
		sb.append(" from (select a.REGION_TREECODE,");
		sb.append(" yj_num,");
		sb.append(" b.user_num user_num");
		sb.append(" from (select t.treecode REGION_TREECODE, sum(num1+num3+num6+num7+num9+num10) yj_num");
