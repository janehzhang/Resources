 ...
 *
 * @author 王晶
 * @description 维度审核中的新增code的操作
 * @date 2012-02-13
 */
public class DimCodeAddImpl implements IDimAuditType {

    private DimAuditDAO dimAuditDAO = new DimAuditDAO();
    private MetaDimTabModHisDAO metaDimTabModHisDAO = new MetaDimTabModHisDAO();
    private Map<Long, Long> parIdMap = new HashMap<Long, Long>();

    /**
     * 维度code的添加方法,1.审核通过,则在维度表中添加数据,
    ...
    public int dimAuditByType(Map<String, Object> dataMap, int auditUserId, int auditFlag, String modMask, Map<String, Object> extraMap) throws Exception {
        if (dataMap != null && dataMap.size() != 0) {
            ...
            if (auditFlag == DimConstant.DIM_HAS_AUDIT_PASS) { //审核通过,写入数据
                ...
                if (parId < 0 && parIdMap.containsKey(parId)) {
                    dataMap.remove("itemParId");
                    dataMap.put("itemParId", parIdMap.get(parId));
                }
                //parCode处理
                ...
                int count = dimAuditDAO.updateDimDataByParId(dimTableName, dimTablePrefix, tableOwner, newItemId, itemId, typeId);//插入维度表之后,更新维度表中原来parId为负数的节点
                if (count == 0) {
                    parIdMap.put(itemId, newItemId);
                }
            ...
            } else if (auditFlag == DimConstant.DIM_HAS_AUDIT_NOT_PASS) { //审核不通过的情况,更改当前节点的子节点的状态
