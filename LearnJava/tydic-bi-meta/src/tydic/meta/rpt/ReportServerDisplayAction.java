 ...
 * @date 2011-12-27
 */
public class ReportServerDisplayAction
{
	/**
	 * 
	 */
	...
	private static Map<String, Object> getMod(ArrayList<Map<String, Object>> rptModules, String modId)
	{
			...
			, String reportDownTab, final HttpServletResponse response) throws Exception
	{
		...
		{
			int outType = 0;
			//		int rptId = Common.parseInt(reportId);
			//		if (reportConfig.trim().equals(""))
			//		{
			//			String[] obj = rptDao.readRptJsCfg(rptId);
			//			if (obj[0].equals("true"))
			//				reportConfig = obj[1];
			//			else
			//				throw new Exception(obj[1]);
			//		}
			...
			Map<String, Object> rptMod = getMod(rptModules, rptTab.get("modId").toString());
			...
			while (rs.next())
			{
				...
				for (int i = 0; i < colNames.length; i++)
				{
					...
					if (colCodeFlag[i])
					{
						Object name = transCodes[i].get(data.toString());
	...
	private void readDuckData(DataAccess access, String duckSQL, Hashtable<String, String> params, int duckLevel, int DUCK_LEVEL,
			...
			boolean[] colCodeFlag, Hashtable[] _transCodes, String[] transCodeSql, String[] rowBackColor, String[] colDisplayAttrs)
			throws Exception
	{
		ResultSet rs = null;
		try
	...
	public Object[] qryGdlByColNames(String[] colNames)
	{
		try
		{
			DataTable table = rptDao.qryGdlByColNames(colNames);
			return table.rows;
		}
		catch (Exception ex)
		{
	...
	public Object[] qryGdlByGdlNames(String[] colNames)
	{
		try
		{
			DataTable table = rptDao.qryGdlByGdlNames(colNames);
			return table.rows;
		}
		catch (Exception ex)
		{
	...
	public Object[] qryMetaTableGdl(long tableId, String colName)
	{
		try
		{
			if (colName.length() > 32)
				return new Object[] { "false", "字段名称超长:\"" + colName + "\"" };
			DataTable table = rptDao.qryMetaTableGdl(tableId, colName);
