...
public class AutoPushEwamOAMsg implements IMetaTimer{
	...
	public void run(String timerName) {
		...
		param.put("method", "sendEWAMDetailData");
		boolean isPush=customerSatisfiedDAO.EwamOAPushCount(new Date());
		if(isPush){
			push(param);
		}
		...
		if(lastDay.getDay()==0){
			//推送前两天的工单
			isPush=customerSatisfiedDAO.EwamOAPushCount(new Date(new Date().getTime()-24 * 60 * 60 * 1000));
			...
			//推送前三天的工单
			isPush=customerSatisfiedDAO.EwamOAPushCount(new Date(new Date().getTime()-2*24 * 60 * 60 * 1000));
